...
const Header: React.FC<HeaderProps> = ({ onMenuToggle, activeModule }) => {
  ...
  const [topBarSettings, setTopBarSettings] = useState<DisplaySettings['topBar']>({
  ...
  // Listen for top bar settings changes from Settings page
  useEffect(() => {
    const handleTopBarSettingsChange = (event: CustomEvent) => {
      setTopBarSettings(event.detail);
    };

    window.addEventListener('topBarSettingsChanged', handleTopBarSettingsChange as EventListener);
    
    return () => {
      window.removeEventListener('topBarSettingsChanged', handleTopBarSettingsChange as EventListener);
    };
  }, []);
  ...
  const loadUserSettings = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        const userTopBarSettings = result.data.display?.topBar;
        if (userTopBarSettings) {
          setTopBarSettings(userTopBarSettings);
        } else {
          // Fallback to legacy settings if topBar is not defined
          const legacySettings: DisplaySettings['topBar'] = {
            showDatabaseStatus: result.data.display?.showDatabaseStatus !== false,
            showSupabaseStatus: true,
            showThemeToggle: result.data.display?.showThemeToggle !== false,
            showNotifications: true,
            showSearch: true,
            showUserProfile: true,
            showMobileSearch: true,
          };
          setTopBarSettings(legacySettings);
        }
      } else {
    ...
    <header className="bg-white dark:bg-dark-800 border-b border-gray-200 dark:border-dark-700 px-6 py-4 transition-colors duration-300">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          {/* Database Status */}
          {topBarSettings.showDatabaseStatus && <DatabaseStatus className="hidden lg:flex" />}
          {/* Supabase Connection Indicator */}
          {topBarSettings.showSupabaseStatus && <SupabaseStatusIndicator />}
          {/* Theme Toggle */}
          {topBarSettings.showThemeToggle && <ThemeToggle />}

          {/* Search */}
          {topBarSettings.showSearch && (
          ...
          {topBarSettings.showMobileSearch && (
          ...
          {/* Notifications */}
          {topBarSettings.showNotifications && <NotificationBell />}

          {/* User menu */}
          {topBarSettings.showUserProfile && (
